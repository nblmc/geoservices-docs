...
The acronym GIS has been used to refer to a number of terms since geospatial technology has come into existence. You may see it refer to "Geographic Information Science," "Geographic Information Software," or "Geographic Information Systems".
...
Raster data can be conceptualized as a grid of squares in which each square or pixel has one value -- that value can represent anything.

This concept is exemplified by elevation data. A Digital Elevation Model (DEM) is a common type of elevation data captured remotely by satellites. The data itself is actually a matrix of numbers, displayed graphically in geospatial software to appear more human-readable. In this graphic view, the value for each pixel signifies the elevation of the area that pixel represents.

Another common raster data you may encounter are georeferenced images. In these cases, pixel values signify the color properties values for each pixel in the image.

Common raster data types are formats you may already be familiar with from working with regular old images -- .pngs, .jpgs, and the most high-resolution of these options, .tiff files. You may also run into GeoTIFFs, which are .tiff files that have been spatially aligned.
...
A **geocoder** is effectively a tool that takes a table with addresses and uses these addressed to estimate latitude and longitude values for each record.
